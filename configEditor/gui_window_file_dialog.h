...
GuiWindowFileDialogState InitGuiWindowFileDialog(const char *initPath)
{
    ...
    // Init window data
    state.windowBounds = (Rectangle){ GetScreenWidth()/2 - 660/2, GetScreenHeight()/2 - 465/2, 660, 465 };
...
# else
        GuiListViewEx((Rectangle){ state->windowBounds.x + 8, state->windowBounds.y + 10 + 20, state->windowBounds.width - 16, state->windowBounds.height - 22 - 16 - 68 }, 
